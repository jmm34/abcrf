...
void findweightsAmelioree(double *ONv, int *inbag, double *NNv, double *WEv, int *nobs, int *nnew, int *ntree, double *thres, int *counti){
    ...
    for ( i=1; i<=(*nnew); ++i ){
      ...
      for ( j=1; j<=(*nobs); ++j ){
		    ...
		    if( ( absol <= (*thres) ) && ( absol >= -(*thres) ) && ( inbag[ j+(k-1)* (*nobs)-1 ] != 0 ) ){
			    counti[ j-1 ] = inbag[ j+(k-1)* (*nobs)-1 ];
			    meancount = meancount + inbag[ j+(k-1)* (*nobs)-1 ];
		    }else{
			    counti[ j-1 ] = 0;
		    }
      }
      if( meancount >= 1 ){
		    for ( j=1 ; j <= (*nobs); ++j ){
				  /* On fait la somme des indicatrices sur le nombre d'éléments dans la feuille */
